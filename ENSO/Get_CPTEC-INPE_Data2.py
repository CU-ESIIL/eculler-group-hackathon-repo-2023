...
if DATA_SRC == "MERGE":
   remote_root_fname = "http://ftp.cptec.inpe.br/modelos/tempo/MERGE/GPM/DAILY/2001/01/MERGE_CPTEC_20010101.grib2"
...
DAT = pd.DataFrame(None, index=np.arange(len(dates)), columns=['date', var_name_output])
...
for idt, dt in enumerate(dates):
   ...
   # Extract individual fields from date being processed
   y = str(dt.year)
   ...
   # Get all data from inside the bbox and calculates the mean
   if DATA_SRC == 'SAMET':
      ds2 = xr.Dataset(ds.sel(lat=slice(lat1, lat2), lon=slice(lon1, lon2)))  # Crop the region of interest
   elif DATA_SRC == 'MERGE':
      ds2 = xr.Dataset(ds.sel(latitude=slice(lat1, lat2), longitude=slice(lon1, lon2)))  # Crop the region of interest
...
DAT.to_csv(f"{var_name_output}.csv", header=True, index=False)                                              # Delete the downloaded file
